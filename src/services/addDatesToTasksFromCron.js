...
const addDatesToTasksFromCron = (tasksJsonObject) => {
  // parse JSON returned from endpoint, then extract data
  const originalTasksObject = JSON.parse(tasksJsonObject);
  const originalTasksArray = originalTasksObject.data;

  // map to return array of task objects
  const updatedTasksArray = originalTasksArray.map(taskObject => {
    ...
    // may need to flatten for React to access our task information, or refactor above
  });
